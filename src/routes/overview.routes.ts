...
// dateSummary=1：额外返回日期汇总桶（报告页用，粒度由 range 决定，后端按东八区分桶）
const QuerySchema = z
  .object({
...
export async function overviewRoutes(fastify: FastifyInstance) {
  fastify.get('/', { onRequest: [fastify.authenticate] }, async (request) => {
    const { range, from, to, lean, dateSummary } = QuerySchema.parse(request.query);
    if (from != null && to != null) {
      ...
      const data = await getOverview(request.user.userId, range, { from, to }, { lean, dateSummary });
      return success(data);
    }
    const data = await getOverview(request.user.userId, range, undefined, { lean, dateSummary });
    return success(data);
  });
}
